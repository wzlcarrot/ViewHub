...
// 使用防抖优化搜索，避免频繁触发
const search = proxy.Utils.debounce(doSearch, 300);
...
onMounted(() => {
  ...
  mitter.on("windowScroll", scrollHandler);
});
...
onUnmounted(() => {
  ...
  if (scrollHandler) {
    mitter.off("windowScroll", scrollHandler);
    scrollHandler = null;
  }
});
...
const logout = () => {
  proxy.Confirm({
    ...
    okfun: async () => {
      ...
      // 清除用户信息，后端会清除HttpOnly Cookie
      loginStore.saveUserInfo({});
      // 不操作localStorage，完全依赖后端的HttpOnly Cookie
    },
  });
};
</script>
